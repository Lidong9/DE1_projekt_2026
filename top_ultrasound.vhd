...
begin
    ...
    debouncer_inst : debounce
        port map (
            clk       => clk,
            rst       => '0',           -- Debouncer samotný neresetujeme
            btn_in    => btn_rst,       -- Syrový vstup z fyzického tlačítka
            btn_state => sig_rst_clean, -- Držená log. 1 po dobu stisku -> náš čistý reset
            btn_press => open           -- 1-taktový puls pro reset nepotřebujeme (necháme nezapojeno)
        );

    -- Měřič ultrazvuku
    ultrasound_inst : Ultrasound_Measurer
        generic map (
            CLK_FREQ => 100_000_000 -- Výchozí hodnota pro Nexys A7
        )
        port map (
            clk      => clk,
            reset    => sig_rst_clean,
    ...
    -- ==========================================
    -- 4. Logika pro Anody (doplnění na 8 cifer)
    -- ==========================================
    -- Spodní 4 cifry řídí display_driver
    an(3 downto 0) <= sig_anode_4bit;
    
    -- Vrchní 4 cifry natvrdo vypneme (společná anoda = 1 znamená vypnuto)
    an(7 downto 4) <= "1111";
